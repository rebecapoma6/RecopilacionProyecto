...
export default function Navbar() {
  ...
  return (
    <nav data-navbar-shell className="app-navbar sticky top-0 z-40 border-b px-4 py-4 md:px-6">
      <div className="mx-auto flex max-w-7xl flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <Link to="/" data-navbar-brand className="flex items-center gap-2 text-2xl font-bold">
          <div data-navbar-logo className="flex h-14 w-14 items-center justify-center">
            <img src={logoPath} alt={t('navbar.logoAlt')} className="h-14 w-14 object-contain" />
          </div>
          ...
          {isAuthenticated ? (
            <>
              <div className="app-chip flex items-center gap-2 rounded-full border px-3 py-2">
                {avatar ? (
                  <img src={avatar} alt={t('navbar.avatarAlt', { user: nombre })} className="h-8 w-8 rounded-full object-cover" />
                ) : (
                  <UserCircle size={24} />
                )}
              ...
              <div className="flex flex-wrap items-center gap-3 text-sm font-medium">
                {!isAdmin && <Link to="/products">{t('navbar.collection')}</Link>}
                <Link to="/perfil">{t('navbar.profile')}</Link>
                {isAdmin && <Link to="/vistaAdmin">{t('navbar.admin')}</Link>}
                {isAdmin && <Link to="/estadisticas">{t('navbar.stats')}</Link>}
              </div>

              <div className="flex items-center gap-3">
                <Button variant="secondary" onClick={cerrarSesion}>{t('navbar.logout')}</Button>
                <button
                  onClick={toggleTheme}
