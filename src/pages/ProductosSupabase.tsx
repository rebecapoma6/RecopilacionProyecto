    ...
    <div className="app-surface mx-auto flex min-h-full w-full max-w-7xl flex-col rounded-[2rem] border p-4 shadow-sm md:p-6">
      <div className="app-surface-strong mb-6 flex flex-col gap-4 rounded-3xl border p-4 shadow-sm sm:flex-row sm:items-end">
        <div className="w-full sm:w-2/3">
          <InputField label={t('products.page.searchLabel')} id="busqueda" name="busqueda" type="text" placeholder={t('products.page.searchPlaceholder')} value={terminoBusqueda} onChange={(e) => setTerminoBusqueda(e.target.value)} />
        </div>

        <div className="flex w-full flex-col sm:w-1/3">
          <label htmlFor="filtro" className="app-muted mb-1 block text-sm font-medium">{t('products.page.filterLabel')}</label>
          <select id="filtro" className="app-input block w-full rounded-xl border p-3" value={criterioFiltro} onChange={(e) => setCriterioFiltro(e.target.value)}>
            <option value="todos">{t('products.filters.any')}</option>
            <option value="titulo">{t('products.filters.title')}</option>
            <option value="autor">{t('products.filters.author')}</option>
            <option value="tipo">{t('products.filters.type')}</option>
          </select>
        </div>
      </div>

      <h2 className="text-3xl font-bold">{t('products.page.title')}</h2>
      <p className="app-muted mb-6">{t('products.page.subtitle')}</p>

      <div className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
        {productosFiltrados.length === 0 && terminoBusqueda !== "" && <p className="app-surface-strong col-span-full rounded-2xl border py-8 text-center">{t('products.page.noResults')}</p>}
        {productos.length === 0 && <p className="app-surface-strong col-span-full rounded-2xl border py-8 text-center">{t('products.page.empty')}</p>}
      ...
      <div className="mt-auto flex justify-center sm:justify-start">
        <Button type="button" onClick={() => navigate("/agregar-items")} className="px-8 py-3">{t('products.page.addNew')}</Button>
      </div>
    </div>
  );
}
